...
(function(window) {
    ...
    var inspect = function (itm) { return (itm.toSource || itm.toString).call(itm) }
...
var compact = (function(arr) {
  ...
  return select(arr, (function(item) {
	...
	var rest = detect(args, (function(arg) {
	    // arg:required
	    return ("rest" === (arg)[0]);
	}));;
	var docString;;
	...
	(function() {
		...
		returnValue = retArr.push(fn.apply(undefined, arr.slice(index, (index + groupSize))));
		returnValue = index += groupSize;
	    }
	    return returnValue;
	})();
	return retArr;
...
$(function () {
	...
	var lisp, js
	if (sibilant.loadNextScript ()) return
	$('script[type="text/lisp"]:not([src])').each (function () {
	    lisp = $(this).text().replace(/(^\s*\/\/\<!\[CDATA\[)|(\/\/\]\]>\s*$)/g, '')
    ...
    scripts = $.makeArray ($('script[type="text/lisp"][src]').map (
	function () {return this.src}))
    
    sibilant.loadNextScript = function () {
	var nextScript = scripts.shift()
	if (nextScript === undefined) return false
	$.get (nextScript, function (data) {
	    console.log (nextScript + " Loaded")
	    var js = sibilant.translateAll (data)
	    try { eval (js) }
	    catch (e) {
		console.log (js)
		throw e
	    }
	    sibilant.scriptLoaded()
	})
	return true
    }

    sibilant.loadNextScript()
})