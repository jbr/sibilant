...
parser.tokenPrecedence = [ "comment", "string", "number", "tick", "hat", "at", "special", "head", "dots", "punctuation", "literal", "argPlaceholder", "otherChar", "openExpression", "closeAndReopen", "closeExpression", "newline", "whitespace", "ignored" ];
parser.orderedRegexes = parser.tokenPrecedence.map((function(x) {
  /* src/parser.sibilant:45:23 */
...
parser.parse = (function parser$parse$(string, context) {
  /* parser.parse src/parser.sibilant:50:0 */
...
var restructure = (function restructure$(input) {
  /* restructure src/restructurer.sibilant:11:0 */
...
var restructurers = (function restructurers$(context, node) {
  /* restructurers src/restructurer.sibilant:34:0 */
...
restructurers.head = (function restructurers$head$(node, context) {
  /* restructurers.head src/restructurer.sibilant:40:0 */
...
restructurers.openExpression = (function restructurers$openExpression$(node, context) {
  /* restructurers.open-expression src/restructurer.sibilant:49:42 */
...
var canClose__QUERY = (function canClose__QUERY$(close, open) {
  /* can-close? src/restructurer.sibilant:60:0 */

  return (acceptablePairs[close.token] === open.token || open.token === ":");
});
restructurers.closeExpression = (function restructurers$closeExpression$(node, context) {
  /* restructurers.close-expression src/restructurer.sibilant:63:0 */

  var first = context.parseStack[0];
  console.log(("closing " + sibilant.prettyPrint(first) + " with " + sibilant.prettyPrint(node)));
  (function() {
  ...
  return (function() {
    if ((first.token === ":" && node.token !== "," && canClose__QUERY(node, context.parseStack[0]))) {
      return restructurers.closeExpression(node, context);
    } else {
      return context;
    }
  }).call(this);
});
restructurers.closeAndReopen = (function restructurers$closeAndReopen$(node, context) {
  /* restructurers.close-and-reopen src/restructurer.sibilant:93:0 */

  var first = context.parseStack[0];
  return (function() {
    if (first.token === ":") {
      return restructurers.closeAndReopen(node, restructurers.closeExpression({
        type: "closeExpression",
        token: ",",
        contents: []
      }, context));
    } else if (first.token === acceptablePairs[node.token[0]]) {
      first.token = node.token[1];
      return context;
    } else {
      throw (new Error(("" + node.token + " cannot close " + sibilant.prettyPrint(first))))
    }
  }).call(this);
});
var openSpecial = (function openSpecial$(node, context) {
  /* open-special src/restructurer.sibilant:105:0 */
...
var acceptSpecials = (function acceptSpecials$(node, context) {
  /* accept-specials src/restructurer.sibilant:118:0 */
...
var acceptIgnoredNodes = (function acceptIgnoredNodes$(node, context) {
  /* accept-ignored-nodes src/restructurer.sibilant:123:0 */
...
var closeSpecials = (function closeSpecials$(node, context) {
  /* close-specials src/restructurer.sibilant:128:0 */
...
sibilant.macros.currentNamespace = (function sibilant$macros$currentNamespace$() {
  /* sibilant.macros.current-namespace src/precompiled-macros.sibilant:16:0 */
...
sibilant.resolveMacro = (function sibilant$resolveMacro$(macroName) {
  /* sibilant.resolve-macro src/precompiled-macros.sibilant:19:0 */
  ...
  return (function() {
    if ((macroName.indexOf("/") !== -1 && 1 < macroName.length && !(macroName.indexOf("\n") !== -1))) {
      ...
      var namespace = detect(sibilant.macros.searchPath, (function(namespace) {
        /* src/precompiled-macros.sibilant:31:33 */
...
sibilant.withDefaultSearchPath = (function sibilant$withDefaultSearchPath$(fn) {
  /* sibilant.with-default-search-path src/precompiled-macros.sibilant:36:0 */
...
sibilant.macros.namespaces.core.includedIn__QUERY = (function includedIn__QUERY$(needle, haystack) {
  /* included-in? src/macros/lists.sibilant:113:0 */

  return "haystack.indexOf(needle) !== -1";
});
sibilant.macros.namespaces.core.excludes__QUERY = (function excludes__QUERY$(haystack, needle) {
  /* excludes? src/macros/lists.sibilant:120:0 */
